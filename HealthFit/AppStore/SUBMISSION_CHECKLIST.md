# Checklist — publicar HealthFit na App Store

Versão alvo: **1.0.10 (Build 23)** · Bundle `luan.com.healthfit.app`

## URLs legais

- Site: https://healthfit-30d87.web.app/
- Privacidade: https://cdn.jsdelivr.net/gh/berglimma/HealhtFit@main/Docs/privacidade/index.html
- Terms of Use (EULA): https://www.apple.com/legal/internet-services/itunes/dev/stdeula/
- Termos HealthFit: https://healthfit-30d87.web.app/termos/
- Suporte: https://healthfit-30d87.web.app/suporte/

## Build

- [x] Versão Xcode **1.0.10 (23)**
- [x] Removido resgate de códigos de cortesia (Guideline 3.1.1)
- [x] `whatsnew.txt` / `review_notes.txt` atualizados
- [x] Archive + upload build **23**
- [x] Attach build 23 na versão 1.0.10
- [x] Reenviado para revisão — **Waiting for Review** (2026-09-23)

## App Store Connect

1. https://appstoreconnect.apple.com/apps/6798621208
2. Versão **1.0.10** com build **23**

## Correção desta revisão

1. Guideline 3.1.1 — removido unlock por promo/courtesy codes no app
